...
if ($UsePSResourceGet)
{
    ...
    $PatCreds = if ($null -eq $env:SYSTEM_ACCESSTOKEN)
    {
        (Get-lhsUserSecret ProdEng-FullPat)
    }
    else
    {
        $ArtifactFeed = "ProdEngGallery"
        $accessToken = $env:SYSTEM_ACCESSTOKEN
        New-Object PSCredential -ArgumentList $ArtifactFeed, (ConvertTo-SecureString $accessToken -AsPlainText -Force)
    }
        ...
        if ($UseModuleFast -or $UsePSResourceGet)
        {
            ...
            if ($UsePSResourceGet)
            {
                ...
                foreach ($requiredModule in $requiredModules)
                {
                    ...
                    $Params = $requiredModule.value.Parameters
                    ...
                    if ($requiredModule.Value -is [System.Collections.Hashtable])
                    {
                        ...
                        if ($requiredModule.Value.Parameters.Credential)
                        {
                            $credString = $requiredModule.Value.Parameters.Credential.ToString()
                            $saveModuleHashtable.Credential = (Invoke-Expression $credString)
                        }

                        if ($requiredModule.Value.Parameters.Repository)
                        {
                            $saveModuleHashtable.Repository = $requiredModule.Value.Parameters.Repository
                        }
                        else
                        {
                            $saveModuleHashtable.Repository = $Gallery
                        }
